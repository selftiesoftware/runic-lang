...
sealed class Lexer(toLowerCase: Boolean) extends NonblockingLexer[Char, Token] {
  ...
  MAIN(id) over { chars => {
    emit(SymbolToken(if (toLowerCase) chars.toLowerCase else chars)(position))
  }
  }

  // Strings
  STRING("\"") = { (string: List[Char], _: List[Char]) => {
    emit(StringToken(string.reverse.mkString)(position))
    MAIN
  }
  }
